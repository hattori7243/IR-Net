...
class my_Conv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        bw = binaryfunction.BinaryQuantize().apply(bw, self.k, self.t)
        bw = binaryfunction.my8BitQuantize().apply(bw)
        ...
        output = F.conv2d(a, bw, self.bias,
                          self.stride, 0,
                          self.dilation, self.groups)
        output += (torch.pow(torch.tensor([2]), torch.tensor([8]))-1).cuda()
        output = output/2
        return output


class IRConv2d_modify(nn.Conv2d):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
        super(IRConv2d_modify, self).__init__(in_channels, out_channels,
                                              kernel_size, stride, padding, dilation, groups, bias)
        self.k = torch.tensor([10]).float().cuda()
        self.t = torch.tensor([0.1]).float().cuda()
    ...
    def forward(self, input):
        ...
        sw = torch.pow(torch.tensor([2]*bw.size(0)).cuda().float(), (torch.log(bw.abs().view(
            bw.size(0), -1).mean(-1)) / math.log(2)).round().float()).view(bw.size(0), 1, 1, 1).detach()
        bw = binaryfunction.BinaryQuantize_modify().apply(bw, self.k, self.t)
        ba = binaryfunction.BinaryQuantize_modify().apply(a, self.k, self.t)
        bw = bw * sw
        output = F.conv2d(ba, bw, self.bias,
                          self.stride, self.padding,
                          self.dilation, self.groups)
