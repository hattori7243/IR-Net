...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                layers += [conv2d,
                           nn.BatchNorm2d(v), nn.Hardtanh(inplace=True)]
            else:
...
class VGG_SMALL_1W1A_modify(nn.Module):

    def __init__(self, num_classes=10):
        super(VGG_SMALL_1W1A_modify, self).__init__()
        self.conv0 = nn.Conv2d(3, 128, kernel_size=3, padding=1, bias=False)
        self.bn0 = nn.BatchNorm2d(128)
        self.conv1 = ir_1w1a.IRConv2d_modify(
            128, 128, kernel_size=3, padding=1, bias=False)
        self.pooling = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bn1 = nn.BatchNorm2d(128)
        # self.nonlinear = nn.ReLU(inplace=True)
        self.nonlinear = nn.Hardtanh(inplace=True)
        self.conv2 = ir_1w1a.IRConv2d_modify(
            128, 256, kernel_size=3, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(256)
        self.conv3 = ir_1w1a.IRConv2d_modify(
            256, 256, kernel_size=3, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(256)
        self.conv4 = ir_1w1a.IRConv2d_modify(
            256, 512, kernel_size=3, padding=1, bias=False)
        self.bn4 = nn.BatchNorm2d(512)
        self.conv5 = ir_1w1a.IRConv2d_modify(
            512, 512, kernel_size=3, padding=1, bias=False)
        self.bn5 = nn.BatchNorm2d(512)
        self.fc = nn.Linear(512*4*4, num_classes)
        self._initialize_weights()
    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:
                    m.bias.data.zero_()
            elif isinstance(m, ir_1w1a.IRConv2d_modify):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
                if m.bias is not None:
                    m.bias.data.zero_()
    ...
    def print_k_t(self):
        print('cov0.k=', self.conv0.k, ',conv0.t=', self.conv0.t)
        print('cov1.k=', self.conv1.k, ',conv1.t=', self.conv1.t)
        print('cov2.k=', self.conv2.k, ',conv2.t=', self.conv2.t)
        print('cov3.k=', self.conv3.k, ',conv3.t=', self.conv3.t)
        print('cov4.k=', self.conv4.k, ',conv4.t=', self.conv4.t)
        print('cov5.k=', self.conv5.k, ',conv5.t=', self.conv5.t)
